...
with tab2:
    ...
    if not gemini_key:
        ...
        page_data = st.session_state.page_data
        text_content = " ".join(page_data.get("paragraphs", []))
                ...
                with st.spinner("AI is thinking..."):
                    try:
                        answer = model.generate_content(
                            f"Based on the following webpage content, answer the question:\n\nContent:\n{text_content[:8000]}\n\nQuestion: {user_q}"
                        )
                        st.write(answer.text)
                    except Exception as e:
                        st.error(f"Error generating answer: {e}")
        else:
            st.warning("⚠️ No paragraph content found in the scraped data.")
