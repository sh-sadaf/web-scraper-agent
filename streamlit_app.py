...
# --- Page Config ---
st.set_page_config(page_title="🌐 Smart Web Scraper", layout="wide", page_icon="🌐")
...
if not scraper_key:
    ...
    tab1, tab2 = st.tabs(["Full Page Scraper", "Topic-Driven Scraper"])

    # --- Full Page Scraper Tab ---
    with tab1:
        url_full = st.text_input("Enter website URL for full page scraping:", key="url_full")
        if st.button("Scrape Full Page"):
            if url_full:
                with st.spinner("Scraping full page..."):
                    try:
                        if not url_full.startswith("http"):
                            url_full = "https://" + url_full
                        params = {"api_key": scraper_key, "url": url_full, "render": "true"}
                        response = requests.get("https://api.scraperapi.com/", params=params, timeout=60)
                        response.raise_for_status()
                        soup = BeautifulSoup(response.content, "html.parser")
    ...
    with tab2:
        url_topic = st.text_input("Enter website URL for topic scraping:", key="url_topic")
        topic = st.text_input("Enter topic/keyword to scrape:", key="topic_input")
        if st.button("Scrape Topic"):
            if url_topic and topic:
                with st.spinner(f"Scraping page for topic '{topic}'..."):
                    try:
                        page_data = scrape_topic(url_topic, topic, scraper_key)
...
if st.session_state.page_data:
    ...
    save_format = st.radio("Choose format:", ["JSON", "CSV"], horizontal=True)
